...
import { DATA_SCOPE_CUSTOM, RESOURCE_TYPE_DATA, RESOURCE_TYPE_MENU } from '@/utils/constants';
...
export const MenuEditForm: React.FC<MenuEditFormProps> = ({ clickOne }) => {
    ...
    useEffect(() => {
        ...
        // 资源类型的现隐
        if (clickOne.resourceType === RESOURCE_TYPE_DATA) {
            setIsShowMenu(false);
        }
        if (clickOne.resourceType === RESOURCE_TYPE_MENU) {
            setIsShowMenu(true);
        }
        // 数据范围的现隐
    ...
    const onChangeResourceType = (e: RadioChangeEvent) => {
        if (e.target.value === RESOURCE_TYPE_DATA) {
            setIsShowMenu(false);
        }
        if (e.target.value === RESOURCE_TYPE_MENU) {
            setIsShowMenu(true);
        }
    };
    // 数据范围变化回调
